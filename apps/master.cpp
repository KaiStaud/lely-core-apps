 ...
 private:
  ...
  OnConfig(std::function<void(std::error_code ec)> res) noexcept override {
    try {
      ...
      std::array<int, 3> vals{{6, 7, 0xF}}; // Double-braces required in C++11 prior to
  ...
  OnRpdoWrite(uint16_t idx, uint8_t subidx) noexcept override {
    if (idx == 0x6041 && subidx == 0) {
      // Obtain the value sent by PDO from object 0x6041:00 on the slave.
      uint32_t val = rpdo_mapped[0x6041][0];
      // Increment the value and store it to an object in the local object
      // dictionary that will be sent by TPDO to object 0x6040:00 on the slave.
      tpdo_mapped[0x6040][0] = 0xF;
