 ...
 protected:
  ...
  OnWrite(uint16_t idx, uint8_t subidx) noexcept override {
    if (idx == 0x6040 && subidx == 0) {
      // Read the value just written to object 0x6040:00, probably by RPDO 1.
      uint32_t val = (*this)[0x6040][0];
      // Copy it to object 0x6041:00, so that it will be sent by the next TPDO.
      (*this)[0x6041][0] = val;
      std::printf("Received %d from master. Sending %d back.\r\n",val,val);
    }
  }
};
